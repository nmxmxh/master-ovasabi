...
// Accepts any dialogue script (object of questions)
export function useCampaignLayout(dialogueScript: Record<string, Question>) {
  const questionsOnly = Object.fromEntries(
    Object.entries(dialogueScript).filter(([key]) => key.startsWith('question'))
  ) as Record<string, Question>;

  // Get campaignId and userId from the current profile (top-level, always up to date)
  const { profile } = useProfile({
    campaignId: 'ovasabi_website', // fallback, will be replaced if profile is loaded
    userId: undefined,
    autoConnect: true
  });
  const campaignId = profile?.campaign_specific?.campaign_id || 'ovasabi_website';
  const userId = profile?.user_id || profile?.guest_id || undefined;
  ...
  const {
    ...
    currentIndex,
    totalQuestions
  } = useQuestionnaire(questionsOnly, campaignId, userId);
  ...
  useGSAP(
    () => {
      ...
      sortedEls.forEach((el, i) => {
        ...
        timeline.fromTo(
          el,
          { autoAlpha: 0, y: -200, rotateZ: randomTilt },
          { autoAlpha: 1, y: 0, rotateZ: 0, duration: 0.5 },
          0.4 + i * 0.06
        );
      });
      timeline
