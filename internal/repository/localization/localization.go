...
func NewLocalizationRepository(db *sql.DB) *Repository {
...
func (r *Repository) CreateTranslation(ctx context.Context, key, language, value string, metadata *commonpb.Metadata) (string, error) {
	meta, err := protojson.Marshal(metadata)
	if err != nil {
		return "", err
	}
	var id int64
	masterID := 0 // TODO: create or resolve master record if needed
	err = r.db.QueryRowContext(ctx,
		`INSERT INTO service_i18n (master_id, key, locale, value, metadata, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, NOW(), NOW()) RETURNING id`,
		masterID, key, language, value, meta,
	).Scan(&id)
	if err != nil {
		return "", err
	}
...
func (r *Repository) GetTranslation(ctx context.Context, translationID string) (*Translation, error) {
	...
	row := r.db.QueryRowContext(ctx,
		`SELECT id, key, locale, value, metadata, created_at FROM service_i18n WHERE id = $1`, translationID)
	if err := row.Scan(&t.ID, &t.Key, &t.Language, &t.Value, &metaRaw, &t.CreatedAt); err != nil {
		if err == sql.ErrNoRows {
...
func (r *Repository) ListTranslations(ctx context.Context, language string, page, pageSize int) ([]*Translation, int, error) {
	if page < 0 {
		page = 0
	}
	if pageSize <= 0 {
	...
	query := `SELECT id, key, locale, value, metadata, created_at FROM service_i18n WHERE locale = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3`
	rows, err := r.db.QueryContext(ctx, query, language, pageSize, offset)
	if err != nil {
		return nil, 0, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var t Translation
		var metaRaw []byte
		if err := rows.Scan(&t.ID, &t.Key, &t.Language, &t.Value, &metaRaw, &t.CreatedAt); err != nil {
			return nil, 0, err
		}
		var meta commonpb.Metadata
		if err := protojson.Unmarshal(metaRaw, &meta); err != nil {
	...
	var total int
	err = r.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM service_i18n WHERE locale = $1`, language).Scan(&total)
	if err != nil {
		total = len(trs)
	}
